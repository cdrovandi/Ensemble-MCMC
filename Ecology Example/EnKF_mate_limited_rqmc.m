function loglike = EnKF_mate_limited_rqmc(theta,y,N)
%%
% EnKF for mate limited model with rqmc extension
% inputs:
%   theta - parameter
%   y - data
...
%%
    ...
    for t = 1:T
        ...
        if (t == 1)
            logn = logn0*ones(N,1);
            % forecase ensemble
            logn = 2*logn + b0 + b1*exp(logn) - log(b2 + exp(logn)) + sigma_e*randn(N,1);
        else
            ...
            % forecase ensemble
            logn = norminv(r(:,2), 2*logn + b0 + b1*exp(logn) - log(b2 + exp(logn)), sigma_e);
        end
