function loglike = EnKF_theta_logistic_rqmc(theta,y,N)
%%
% EnKF for theta logistic model with rqmc extension
% inputs:
%   theta - parameter
%   y - data
...
%%
    ...
    for t = 1:T
        ...
        if (t == 1)
            logn = logn0*ones(N,1);
            % forecase ensemble
            logn = logn + b0 + b1*exp(logn).^b2 + sigma_e*randn(N,1);
        else
            ...
            % forecase ensemble
            logn = norminv(r(:,2), logn + b0 + b1*exp(logn).^b2, sigma_e);
        end
