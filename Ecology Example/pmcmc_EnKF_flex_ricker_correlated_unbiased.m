function [theta, loglikes] = pmcmc_EnKF_flex_ricker_correlated_unbiased(y,theta_init,iters,N,cov_rw,sigma_corr)
%%
% eMCMC for flex ricker model with correlated and unbiased extensions
% inputs:
%   y - data
...
loglike_curr =  EnKF_flex_ricker_fixed_unbiased([theta_curr(1) theta_curr(2) theta_curr(3) exp(theta_curr(4)) exp(theta_curr(5)) theta_curr(6)],y,N,r);
...
i    
    ...
    loglike_prop =   EnKF_flex_ricker_fixed_unbiased([theta_prop(1) theta_prop(2) theta_prop(3) exp(theta_prop(4)) exp(theta_prop(5)) theta_prop(6)],y,N,r_prop);
